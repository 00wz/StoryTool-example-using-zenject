...
{    
    ...
    private IEnumerator AttackRoutine()
    {
        ...
        try
        {
            var target = Physics.OverlapSphere(transform.TransformPoint(attackAreaCenter), attackAreaRadius)
                .FirstOrDefault(collider => collider.GetComponentInParent<Health>() != null);
            yield return _attack.PerformAttackRoutine(target?.GetComponentInParent<Health>());
        }
        finally
        {
            isAttacking = false;
